...
When a retail `.nds` game is launched from Pico Launcher, you can return to Pico Launcher in-game with `DOWN + START + SELECT`. Pico Loader will also write a `<rom>.state.bin` snapshot containing RAM, shared/ARM7 WRAM, VRAM, ARM9 ITCM/DTCM, CPU context, palette, OAM, and partial ARM9/ARM7 IO state including display, DMA, timers, ARM9 TCM control, and ARM7 audio state before returning. Recent snapshots also include ROM identity metadata so mismatched resume attempts can be rejected.
